...
def perform_action(action, env,kb,planned_actions,obs):
    ...
    if name == 'eat': 
        ...
        if done : return obs, reward, done, info , planned_actions
        # Example message:
...
def process_state(obs: dict, kb: Prolog, in_battle:bool = True):
    ...
    for item in obs['inv_strs']:
        ...
        if 'weapon in hand' in item:
            ...
            objs=list(filter(lambda x: x in wp,AVAIABLE_WEAPONS))
            if len(objs)!=0:
                wp=objs[0]
        ...
        if 'being worn' in item: #the armor the agent is wearing
            ...
            if ar not in AVAIABLE_ARMORS:
                kb.retractall(f"""armor_stats('{ar}',_)""")
                kb.assertz(f"""armor_stats('{ar}',0)""")
            continue
